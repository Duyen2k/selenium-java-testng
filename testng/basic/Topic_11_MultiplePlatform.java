package basic;

import org.openqa.selenium.By;
import org.openqa.selenium.Platform;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.edge.EdgeDriver;
import org.openqa.selenium.firefox.FirefoxDriver;
import org.testng.Assert;
import org.testng.annotations.*;

import java.time.Duration;

public class Topic_11_MultiplePlatform {
    WebDriver driver;

    String environmentURL;
    Platform platform;
    By emailTextbox = By.cssSelector("input#email");
    By passwordTextbox = By.cssSelector("input#pass");
    By loginButton = By.cssSelector("button#send2");

    @Parameters({"browser","environmentName","platformName"})
    @BeforeClass
    public void initialBrowser(String browserName,String environmentName, @Optional("Windows") String platformName) {
        System.out.println("Browser name ="+browserName);
        System.out.println("Environment name ="+environmentName);
        System.out.println("Platform name ="+platformName);

        //Dùng Optional để tạo phuong án backup, khi platformName ko có giá trị thì nó mặc định lấy "Windows"

        //tạo browser để chạy với biến browserName
       switch (browserName.toUpperCase()){
           case "FIREFOX":
               driver=new FirefoxDriver();
               break;
           case "CHROME":
               driver=new ChromeDriver();
               break;
           case "EDGE":
               driver=new EdgeDriver();
               break;
           default:
               throw new RuntimeException("browser name is not support");


       }

       //tạo môi trường với biến environmentName
        switch (environmentName.toUpperCase()) {
            case "DEV":
                environmentURL="http://dev.live.techpanda.org";
                break;
            case "TESTING":
                environmentURL="http://testing.live.techpanda.org";
                break;
            case "STAGING":
                environmentURL="http://staging.live.techpanda.org";
                break;
            case "PRODUCTION":
                environmentURL="http://live.techpanda.org";
                break;

            default:
                throw new RuntimeException("Environment name is not Support");
        }

        //
        switch (platformName.toUpperCase()) {
            case "WINDOWS":
                platform= Platform.WINDOWS;
                break;
            case "MAC":
                platform= Platform.MAC;
                break;
            case "LINUX":
                platform= Platform.LINUX;
                break;
            case "ANDROID":
                platform= Platform.ANDROID;
                break;
            case  "IOS":
                platform=Platform.IOS;
            default:
                throw new RuntimeException("Platform name is not Support");
        }

        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(15));
        driver.get(environmentURL+"/index.php/customer/account/login/");
    }

    @Test(dataProvider = "loginData")
    public void shouldBeLoginToSystem(String username, String password, String zipCode) {
//        driver.get("http://live.techpanda.org/index.php/customer/account/login/");

        driver.findElement(emailTextbox).sendKeys(username);
        driver.findElement(passwordTextbox).sendKeys(password);
        driver.findElement(loginButton).click();
        Assert.assertTrue(driver.findElement(By.xpath("//div[@class='col-1']//p")).getText().contains(username));

        // Mua Hàng

        // Thanh Toán

        // Ship Hàng

        driver.findElement(By.xpath("//header[@id='header']//span[text()='Account']")).click();
        driver.findElement(By.xpath("//a[text()='Log Out']")).click();
    }

    @DataProvider(name = "loginData")
    public Object[][] UserAndPasswordData() {
        //Màng 2 chiều, ánh xạ data lên TC bên trên. Dù có 1TC nhưng sẽ chạy với 3 data
        return new Object[][]{
                {"selenium_11_01@gmail.com", "111111", "65000"}
        };
    }

    @AfterClass
    public void cleanBrowser() {
        driver.quit();
    }


}
